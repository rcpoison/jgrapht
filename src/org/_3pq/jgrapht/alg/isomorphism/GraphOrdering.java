 ...
 * vertexes: 1,2,3,4
 * edges:1->2 , 3->4 ,1->1
 * Implementation as imutable graph by int[] for vetexes and 
 * LabelsEdge[] for edges.
 * 
 *  The current algoritms do not support graph with multiple edges (Multigraph / Pseudograph)
 *   For the maintaner: The reason for it is the use of edges sets of LabelsEdge in which
 *   the equals checks for source and target vertexes. Thus there cannot be two LabelsEdge
 *   with the same source and target in the same Set. 
 *  
 *	@author Assaf
 *	@since	May 20, 2005
 *
...
public class GraphOrdering {
	...
	private class LabelsEdge
	{
		...
		/** checks both source and target.does not check class type
		 * to be fast , so it may throw ClassCastException. carefull!
	...
	/** holds a mapping between key=Object(vertex) and value=Integer(vertex order)
	 * It can be used for identifying the order of regular vertex/edge .
	 */
	...
	private void init (Set vertexSet,Set edgeSet)
	{
		...
		for (Iterator iter = edgeSet.iterator(); iter.hasNext();) 
		{
			...
			int targetLabel = ((Integer)(mapVertexToOrder.get(edge.getTarget()))).intValue();
	...
	/**
	 * creates a new labels graph according to the regular graph. After it`s creation
	 * they will no longer be linked , thus changes on one will not effect the other. 
	 * @param regularGraph
	 * @return
	 */
	...
	/** 
	 * test equality by order of edges 
	 */
	public boolean equalsByEdgeOrder(GraphOrdering otherGraph)
	{
		boolean result = this.getLabelsEdgesSet().equals(otherGraph.getLabelsEdgesSet());
