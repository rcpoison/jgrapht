 ...
 * -------------------
 * (C) Copyright 2005, by Ewgenij Proschak and Contributors.
 *
 ...
 * comparative Modeling of Protein Structure; J.Mol. Biol. (1998); vol 279;
 * pp. 287-302]
 *    
...
public class BronKerboschCliqueFinder<V, E extends Edge<V>> {
     ...
     *
     * @param graph the graph in which cliques are to be found; graph
     * must be simple
     */
    ...
    /**
     * Finds all maximal cliques of the graph.  A clique is maximal
     * if it is impossible to enlarge it by adding another vertex
     * from the graph.  Note that a maximal clique is not necessarily
     * the biggest clique in the graph.
     *
     * @return Collection of cliques (each of which is represented as a
     * Set of vertices)
     */
    public Collection<Set<V>> getAllMaximalCliques(){
        // TODO jvs 26-July-2005:  assert that graph is simple
        
        m_cliques = new ArrayList<Set<V>>();
        List<V> potential_clique = new ArrayList<V>();
        List<V> candidates = new ArrayList<V>();
        List<V> already_found = new ArrayList<V>();
        candidates.addAll(m_graph.vertexSet());
    ...
    public Collection<Set<V>> getBiggestMaximalCliques(){
        ...
        int maximum = 0;
        Collection<Set<V>> biggest_cliques =
            new ArrayList<Set<V>>();
        for (Set<V> clique : m_cliques) {
    ...
    private void findCliques(
        List<V> potential_clique, List<V> candidates,
        ...
        if (!end(candidates,already_found)){
            ...
            for(int i=0;i<candidates_array.length;i++){
                ...
                for(V new_found : already_found) {
                    if (m_graph.containsEdge(candidate,new_found)){
                ...
                if (new_candidates.isEmpty()&& new_already_found.isEmpty()){
                    ...
                    m_cliques.add(new HashSet<V>(potential_clique));
                }//of if
                else{
                    //recursive call
                    findCliques(
                        potential_clique, new_candidates, new_already_found);
                }//of else
    ...
    private boolean end(List<V> candidates, List<V> already_found){
        ...
        for(V found : already_found) {
            ...
            for(V candidate : candidates) {
                if (m_graph.containsEdge(found,candidate)){
                    edgecounter++;
                }//of if
            }//of for
