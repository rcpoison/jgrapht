...
/**
 * <p>
 * Complements the {@link org._3pq.jgrapht.alg.ConnectivityInspector} class with the capability to
 * compute the strongly connected components of a directed graph. The algorithm
 * is implemented after "Corman et al: Introduction to agorithms", Chapter 25.2.
 * It has a running time of O(V + E).
 * </p>
 * <p>
 * Unlike {@link org._3pq.jgrapht.alg.ConnectivityInspector}, this class does not implement incremental
...
public class StrongConnectivityInspector {
     ...
     * {@link StrongConnectivityInspector#stronglyConnectedSets()}. If you
     * don't need subgraphs, use that method.
     * </p>
     * 
    ...
    public List stronglyConnectedSubgraphs() {
        if (m_stronglyConnectedSubgraphs == null) {
            ...
            while (iter.hasNext()) {
                m_stronglyConnectedSubgraphs.add(new DirectedSubgraph(m_graph,
                        (Set) iter.next(), null));
            }
        }
    ...
    private void dfsVisit(DirectedGraph graph, VertexData vertexData,
        ...
        while (!stack.isEmpty()) {
            ...
            if (!data.m_discovered) {
                ...
                // follow all edges
                Iterator iter = graph.outgoingEdgesOf(data.m_vertex).iterator();

                while (iter.hasNext()) {
                    DirectedEdge edge = (DirectedEdge) iter.next();
                    VertexData targetData = (VertexData) m_vertexToVertexData
                            .get(edge.getTarget());
    ...
    private void createVertexData() {
        ...
        while (iter.hasNext()) {
            ...
            m_vertexToVertexData.put(vertex, new VertexData(vertex, false,
                    false));
        }
    }
