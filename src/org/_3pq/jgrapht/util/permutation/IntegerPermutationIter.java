...
/**
 *  Itertate through permutation of N elements.
 *  <li>use getNext() to get the next permutation order ,  for example(N=4): perm0=[1,2,3,4] 
 *  perm1=[1,2,4,3] perm2=[1,3,2,4] .
 *  <li>use hasNext() or verify by counter<getTotalNumberOfPermutations() that you do not
 *  overflow the max number. RunTimeException will be thrown if you do.
 *  
 *  getting the next permutation is done in O(N)
...
public class IntegerPermutationIter implements Iterator,ArrayPermutationsIter{
	...
	/** will hold the current value. will be cahnged only by getNext() */
	private int[] currentValueBackup;
	
	/**
	 * creates an array of size N with elements 1,2,...,n-1
	 * Usefull for describing regular permutations.
	 * see IntegerPermutationIter(int[] array) for the other kind of permutations
	 * Efficency: of initiation is O(N)
	 * @param N
	 */
	...
	/** 
	 * uses a predifined array (sorted), for example:
	 * [3,1,1,2,1]-->[1,1,1,2,3] note that there are much less than
	 * 5! premutations here , because of the repetative 1s.
	 * @param array creates a copy of it (so sort / later changes will not matter)
	 */
	...
	/**
	 * efficency: O(N)
	 * implementation , try to take the next!
	 * @return
	 */
	...
	/**
	 * fasade. use it with getNext.
	 * efficency: O(N)
	 * @return a new Array with the permutatation order. for example: perm0=[1,2,3,4] 
	 * perm1=[1,2,4,3] perm2=[1,3,2,4] 
	 * 
	 * 
	 */
	...
	/** utility method to convert the array into a string
	 * examples: [] [0] [0,1][1,0]
	 * @param array
	 * @return
	 */
