 ...
 * BlockCutpointGraph} for more information.  A biconnected graph
 * has only one block (i.e. no cutpoints).
 *
...
public class BiconnectivityInspector<V,E> {
    ...
    public Set<Set<V>> getBiconnectedVertexComponents() {
        ...
        for (Iterator<UndirectedGraph<V,E>> iter = this.blockCutpointGraph.vertexSet().iterator(); iter
                .hasNext();) {
    ...
    public Set<Set<V>> getBiconnectedVertexComponents(V vertex) {
        ...
        for (Iterator<Set<V>> iter = getBiconnectedVertexComponents().iterator(); iter
                .hasNext();) {
