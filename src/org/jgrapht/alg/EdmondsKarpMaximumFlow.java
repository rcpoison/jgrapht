...
/**
 * A <a href = "http://en.wikipedia.org/wiki/Flow_network">flow network</a> is a directed graph where each edge has a capacity
 * and each edge receives a flow. The amount of flow on an edge can not exceed the capacity of the edge
 * (note, that all capacities must be non-negative).
 * A flow must satisfy the restriction that the amount of flow into a vertex equals the amount of flow out of it,
 * except when it is a source, which "produces" flow, or sink, which "consumes" flow.
 * 
 * <p>This class computes maximum flow in a network using
 * <a href = "http://en.wikipedia.org/wiki/Edmonds-Karp_algorithm">Edmonds-Karp algorithm</a>.
 * Be careful: for large networks this algorithm may consume significant amount of time (its upper-bound complexity is O(VE^2),
 * where V - amount of vertices, E - amount of edges in the network).
 * 
 * <p>For more details see Andrew V. Goldberg's <i>Combinatorial Optimization (Lecture Notes)</i>.
 */
public final class EdmondsKarpMaximumFlow<V, E>
{
	 ...
	 * Current source and sink are set to <tt>null</tt>.
	 * If <tt>network</tt> is weighted, then capacities are weights, otherwise all capacities are equal to one.
	 * Doubles are compared using <tt>DEFAULT_EPSILON</tt> tolerance.
	 * 
	 * @param network network, where maximum flow will be calculated
	 */
	public EdmondsKarpMaximumFlow(
	 ...
	 * Note, that <tt>source</tt> and <tt>sink</tt> must be vertices of the <tt>network</tt> passed to the constructor,
			...
			for (Arc currentArc: nodes.get(currentNode).outgoingArcs)
			{
				if (currentArc.flow + epsilon < currentArc.capacity)
				{
					if (!nodes.get(currentArc.head).was)
					{
						...
						nodes.get(currentArc.head).flowAmount = Math.min(
	...
	/**
	 * Returns maximum flow value, that was calculated during last <tt>calculateMaximumFlow</tt> call, or
	 * <tt>null</tt>, if there was no <tt>calculateMaximumFlow</tt> calls.
	 * 
	...
	/**
	 * Returns maximum flow, that was calculated during last <tt>calculateMaximumFlow</tt> call, or
	 * <tt>null</tt>, if there was no <tt>calculateMaximumFlow</tt> calls.
	 * 
	...
	/**
	 * Returns current source vertex, or <tt>null</tt> if there was no <tt>calculateMaximumFlow</tt> calls.
	 * 
	...
	/**
	 * Returns current sink vertex, or <tt>null</tt> if there was no <tt>calculateMaximumFlow</tt> calls.
	 * 
	 * @return current sink
	 */	
	...
	class Node
	{
		...
		List<Arc> outgoingArcs = new ArrayList<Arc>(); // list of outgoing arcs in the residual network
	...
	class Arc
	{
		...
		Arc reversed;         // for each arc in the original network we are to create reversed arc
		E prototype;          // corresponding edge in the original network, can be null, if it is reversed arc
