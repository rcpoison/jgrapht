 ...
 * Wagner minimum cut algorithm</a>.  Deterministically computes the minimum
 * cut in O(|V||E| + |V|log|V|) time.  This implementation uses Java's
 * PriorityQueue and requires O(|V||E|log|E|) time.
 * 
...
public class StoerWagnerMinimumCut<V, E> {
    ...
    public StoerWagnerMinimumCut(WeightedGraph<V, E> graph){
        ...
        workingGraph = new SimpleWeightedGraph<Set<V>, DefaultWeightedEdge>(
        ...
        while(workingGraph.vertexSet().size() > 2) minimumCutPhase(a);
    ...
    protected void minimumCutPhase(Set<V> a){
        ...
        while(!queue.isEmpty()){
            ...
            for( DefaultWeightedEdge e : workingGraph.edgesOf(v) ){
                Set<V> vc;
                if( v != workingGraph.getEdgeSource(e) ) vc = workingGraph.getEdgeSource(e); 
                else vc = workingGraph.getEdgeTarget(e);
                if(dmap.get(vc) != null){
    ...
    protected VertexAndWeight mergeVertices(Set<V> s, Set<V> t){
        ...
        for( Set<V> v : workingGraph.vertexSet() ){
            if(s != v  && t != v){
                ...
                if(etv != null)  wtv = workingGraph.getEdgeWeight(etv);
                if(esv != null)  wsv = workingGraph.getEdgeWeight(esv);
                double neww = wtv + wsv;
                wsum += neww;
                if(neww != 0.0) 
