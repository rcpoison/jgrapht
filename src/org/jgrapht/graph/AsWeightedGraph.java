 ...
 * 
 * <p>Query operations on this graph "read through" to the backing
 * graph. Vertex addition/removal and edge addition/removal are all supported
 * (and immediately reflected in the backing graph).  Setting an edge weight
 * will pass the operation to the backing graph as well if the backing graph
 * implements the WeightedGraph interface.  Setting an edge weight will modify
 * the weight map in order to maintain a consistent graph.</p>
 *
    ...
    implements Serializable, WeightedGraph<V, E>
{
     ...
     *
     * @param g the backing graph over which a weighted view is to
     * be created.
     * @param weightMap A mapping of edges to weights.  If an edge is
     * not present in the weight map, the edge weight for the underlying
     * graph is returned.  Note that a live reference to this map
     * is retained, so if the caller changes the map after construction,
     * the changes will affect the AsWeightedGraph instance as well.
     */
