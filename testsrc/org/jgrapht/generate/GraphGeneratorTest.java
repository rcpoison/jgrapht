    ...
    extends TestCase
{
    ...
    public void testGridGraphGenerator() {
		...
		GridGraphGenerator<String, String> generator = new GridGraphGenerator<String, String>(rows, cols);
		...
		resultMap.clear();
		Graph<String, String> undirectedGridGraph = new SimpleGraph<String, String>(new StringEdgeFactory());
    ...
    public void validateGridGraphGenerator(int rows, int cols, Graph<String, String> gridGraph, Map<String, String> resultMap) {
    	...
    	assertEquals("number of vertices is wrong (" + gridGraph.vertexSet().size() +
    			"), should be " + expectedVerticeNum, expectedVerticeNum, gridGraph.vertexSet().size());
    	int expectedEdgesNum = ((rows - 1) * cols + (cols - 1) * rows) * (gridGraph instanceof UndirectedGraph ? 1 : 2);
    	assertEquals("number of edges is wrong (" + gridGraph.edgeSet().size()
    			+ "), should be " + expectedEdgesNum, expectedEdgesNum, gridGraph.edgeSet().size());
    	...
    	for (String v : gridGraph.vertexSet()) {
    		...
    		assertTrue("vertex with illegal number of neighbors (" + neighborsSize + ").",
    	...
    	assertEquals("there should be exactly " + expCornerVertices + " corner (with two neighbors) vertices. " 
    			+ " actual number is " + cornerVertices + ".", expCornerVertices, cornerVertices);
    	assertEquals("there should be exactly " + expBorderVertices + " border (with three neighbors) vertices. " 
    			+ " actual number is " + borderVertices + ".", expBorderVertices, borderVertices);
    	assertEquals("there should be exactly " + expInnerVertices + " inner (with four neighbors) vertices. "
    			+ " actual number is " + innerVertices + ".", expInnerVertices, innerVertices);
    	
    	//result map validations
    	Set<String> keys = resultMap.keySet();
    	assertEquals("result map contains should contains exactly 4 corner verices",
    			4, keys.size());
    	...
    	for (String key : keys) {
    		...
    		neighborsSize = neighbors.size(); 
    		assertEquals("corner vertex should have exactly 2 neighbors",
    				2, neighborsSize);
    	}
    }
}
