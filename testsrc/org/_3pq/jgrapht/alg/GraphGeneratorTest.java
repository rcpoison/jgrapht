 ...
 * Changes
 * -------
 * 17-Sept-2003 : Initial revision (JVS);
 *
 */
...
import org._3pq.jgrapht.alg.generator.CycleGraphGenerator;
...
public class GraphGeneratorTest extends TestCase {
     ...
     * .
     */
    public void testCycleGraphGenerator(  ) {
        GraphGenerator gen       = new CycleGraphGenerator( DEGREE );
        DirectedGraph  g         = new DefaultDirectedGraph(  );
        Map            resultMap = new HashMap(  );
        gen.generateGraph( g, m_vertexFactory, resultMap );
        assertEquals( DEGREE, g.vertexSet(  ).size(  ) );
        assertEquals( DEGREE, g.edgeSet(  ).size(  ) );
    ...
    public void testEmptyGraphGenerator(  ) {
        GraphGenerator gen       = new EmptyGraphGenerator( DEGREE );
        DirectedGraph  g         = new DefaultDirectedGraph(  );
        Map            resultMap = new HashMap(  );
        gen.generateGraph( g, m_vertexFactory, resultMap );
        assertEquals( DEGREE, g.vertexSet(  ).size(  ) );
    ...
    public void testLinearGraphGenerator(  ) {
        GraphGenerator gen       = new LinearGraphGenerator( DEGREE );
        DirectedGraph  g         = new DefaultDirectedGraph(  );
        Map            resultMap = new HashMap(  );
        gen.generateGraph( g, m_vertexFactory, resultMap );
        assertEquals( DEGREE, g.vertexSet(  ).size(  ) );
        assertEquals( DEGREE - 1, g.edgeSet(  ).size(  ) );
